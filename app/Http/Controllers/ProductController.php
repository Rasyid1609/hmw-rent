...
class ProductController extends Controller
{
    ...
    public function index(): Response
    {
        ...
        ->select(['id', 'prod_code', 'title', 'slug', 'description', 'cover', 'price_prod', 'status', 'price', 'category_id', 'brand_id', 'created_at'])
    ...
    private function productCode(int $publication_year, int $category_id): string
    {
        ...
        $prod_code_prefix = 'CA'. $publication_year. '.' . str()->slug($category->name). '.';

        $last_product = Product::query()
            ->where('prod_code', 'like', $prod_code_prefix . '%')
        ...
        if ($last_product) {
            $last_order = (int) substr($last_product->prod_code, -4);
            $order = $last_order + 1;
        }

        $ordering = str_pad($order, 4, '0', STR_PAD_LEFT);
        return $prod_code_prefix . $ordering;
    }
}
